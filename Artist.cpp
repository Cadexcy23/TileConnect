...
bool Artist::init()
{
	...
	else
	{
		...
		//Create window
		gWindow = SDL_CreateWindow("TileConnect", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, Artist::SCREEN_WIDTH, Artist::SCREEN_HEIGHT, SDL_WINDOW_SHOWN);
		Artist artist;
...
void Artist::drawImage(SDL_Texture* tex, int x, int y, int w, int h, SDL_Rect* source, double r, SDL_Point* center, SDL_RendererFlip flip, bool scaled)
{
	SDL_Rect img;
	...
	if (scaled)
		...
		img.h = drawH;
	SDL_RenderCopyEx(gRenderer, tex, source, &img, r, center, flip);
}
